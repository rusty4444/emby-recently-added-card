# Emby Recently Added Card

A custom Home Assistant Lovelace card that shows your recently added movies and TV shows from Emby. Auto-cycles through items with poster art, blurred background, synopsis, ratings, and color-coded indicators.
...
<p align="center">
  <img src="screenshots/recently-added.jpg" alt="Emby Recently Added Card" width="600">
</p>

## Features

- Displays the 5 most recently added movies and 5 most recently added TV shows from Emby
...
- Connects directly to your Emby server via its API
...
3. Enter `https://github.com/rusty4444/emby-recently-added-card` and select **Dashboard** as the category
4. Click **Add**
5. Search for "Emby Recently Added Card" in HACS and click **Install**
...
1. Download `emby-recently-added-card.js` from the [latest release](https://github.com/rusty4444/emby-recently-added-card/releases/latest)
2. Place it in your `<config>/www/` directory
3. Go to **Settings → Dashboards** → three dots (top right) → **Resources**
4. Click **Add Resource**
5. URL: `/local/emby-recently-added-card.js`
...
```yaml
type: custom:emby-recently-added-card
...
|--------|------|---------|-------------|
| `emby_url` | string | **Required** | Your Emby server URL (e.g., `http://192.168.1.100:8096`) |
| `api_key` | string | **Required** | Your Emby API key (see below) |
| `user_id` | string | optional | Your Emby user ID (see below — leave blank to auto-detect the first user) |
...
## How to Get Your Emby API Key

1. Open the Emby web interface and log in as an administrator
...
**Option A — From the Emby dashboard:**
1. Go to **Dashboard** → **Users**
2. Click on your user
3. The user ID is the long string at the end of the URL (e.g., `.../Users/abc123def456...`)

**Option B — Auto-detect:**
Leave `user_id` blank in your card config and the card will automatically use the first user returned by your Emby server.

---

## How It Works

- Connects directly to the Emby API using your server URL and API key
- Fetches recently added movies and TV episodes from your Emby library
...
The card uses Emby's `ProviderIds` (TMDB and IMDB IDs included in each item's metadata) to look up trailers via the TMDB API.
...
- **No items showing**: Double-check your `emby_url` and `api_key`. Make sure the Emby server is reachable from the device viewing the dashboard and that the API key is valid.
- **CORS errors in browser console**: Emby must be reachable from the same network as the device viewing the dashboard. Ensure the port (default 8096) is not blocked by a firewall.
...
- [jellyfin-recently-added-card](https://github.com/rusty4444/jellyfin-recently-added-card) — the Jellyfin version of this project
