 ...
 * Custom Lovelace card that displays the latest movies and TV shows from Emby
 * with interleaved movie/show/movie/show cycling and cinematic transitions.
 *
 * Adapted from jellyfin-recently-added-card by Perplexity Computer.
 */

class EmbyRecentlyAddedCard extends HTMLElement {
  constructor() {
    super();
  ...
  async _fetchData() {
    try {
      ...
      const movieItems = movies.map((item) => {
        ...
        // Check if the item actually has a Backdrop image
        const hasBackdrop =
      ...
      console.warn('Emby Recently Added Card: Fetch error', err);
      const errEl = this.shadowRoot.querySelector('.error-msg');
      if (errEl) {
        errEl.textContent = `Could not connect to Emby: ${err.message}`;
  ...
  _updateDisplay() {
    ...
    // Dots — gold for movies, Emby green for TV
    if (dotsEl) {
  ...
  async _fetchTvTrailer(seriesId, seasonNumber) {
    ...
    try {
      ...
      // Step 1: Fetch series metadata from Emby to get TMDB ID
      const seriesResp = await fetch(
        `${base}/Users/${userId}/Items/${seriesId}?Fields=ProviderIds&api_key=${key}`,
        { headers: { Accept: 'application/json' } }
      );
      if (!seriesResp.ok) throw new Error(`Emby series metadata HTTP ${seriesResp.status}`);
      ...
      console.warn('Emby Recently Added Card: TV trailer fetch error', err);
      this._trailerCache[cacheKey] = null;
      return null;
    }
  }
  ...
  async _fetchTrailer(tmdbId, imdbId) {
    ...
    if (!resolvedTmdbId && imdbId) {
      ...
      try {
        ...
        console.warn('Emby Card: TMDB find lookup failed', e);
      }
      if (!resolvedTmdbId) {
        this._trailerCache[cacheKey] = null;
        return null;
    ...
    try {
      ...
      console.warn('Emby Card: TMDB videos fetch failed', e);
      this._trailerCache[cacheKey] = null;
      return null;
    }
  }
  ...
  _render() {
    ...
    // Emby logo — "e" letterform in a green rounded rectangle (brand color #52B54B)
    const embyLogo = `
      <svg class="emby-logo" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32" aria-label="Emby">
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        .dot.active.tv {
          ...
          box-shadow: 0 0 6px rgba(82, 181, 75, 0.4);
          width: 18px;
          border-radius: 3px;
        }
  ...
  static getConfigForm() {
    return {
      ...
      computeHelper: (schema) => {
        const helpers = {
          emby_url: 'e.g. http://192.168.1.100:8096',
          api_key: 'Emby Dashboard → Advanced → API Keys',
...
customElements.define('emby-recently-added-card', EmbyRecentlyAddedCard);

window.customCards = window.customCards || [];
window.customCards.push({
  type: 'emby-recently-added-card',
  name: 'Emby Recently Added',
  description: 'Auto-cycling display of recently added Emby media — movies and TV shows.',
});
